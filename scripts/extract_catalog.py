...
PAGES_DIR = SCRIPTS_DIR / "catalog_pages"
...
def download_pages(max_pages: int) -> None:
...
def process_pages(max_pages: int, output_path: Path, start_page: int = 1) -> None:
    pages_out = []
    for n in range(1, max_pages + 1):
...
def main():
    parser = argparse.ArgumentParser(description="Extract Jake's Fireworks catalog from Issuu")
    parser.add_argument("--pages", type=int, default=TOTAL_PAGES, help="Number of pages to process (default: 177)")
    parser.add_argument("--start-page", type=int, default=PRODUCT_START_PAGE, help="First page with product data (default: 11)")
    parser.add_argument("--skip-download", action="store_true", help="Skip download phase")
    parser.add_argument("--output", type=Path, default=SCRIPTS_DIR / "jakes_catalog_vision.json", help="Output JSON path")
    args = parser.parse_args()

    max_pages = min(args.pages, TOTAL_PAGES)

    if not args.skip_download:
        print(f"=== Download phase ({max_pages} pages) ===")
        download_pages(max_pages)

    print(f"\n=== OCR/parse phase ({max_pages} pages, products from page {args.start_page}) ===")
    process_pages(max_pages, args.output, start_page=args.start_page)
